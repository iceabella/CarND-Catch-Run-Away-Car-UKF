...
UKF::UKF() {
  ...
  // Fixed time step interval used for prediction, defined in ukf.h
  //dt_ = 0.1;
  ...
  // covariance matrix for predicted state n_t_ time steps
  P_nt_ = MatrixXd(n_x_, n_x_);
  P_nt_.fill(0.0);

  // Process noise standard deviation longitudinal acceleration in m/s^2
  std_a_ = 3;	// tunable parameter
...
void UKF::Prediction_nt(int n_t) {
  ...
  vector, x_nt_. Predict sigma points, the state, and the state covariance matrix.
  ...
  */
  ...
  for(int i=0; i<n_t; i++){
  	...
  	//create augmented mean state
  	x_aug.head(n_x_) = x_nt_;
  	x_aug(n_x_) = 0;
  	x_aug(n_x_+1) = 0;

  	//create augmented covariance matrix
  	P_aug.topLeftCorner(n_x_,n_x_) = P_nt_;
  	P_aug(n_x_,n_x_) = std_a_*std_a_;
  	P_aug(n_x_+1,n_x_+1) = std_yawdd_*std_yawdd_;
  	//create square root matrix
  	MatrixXd A = P_aug.llt().matrixL();
  	...
  	x_nt_.fill(0.0);
  	for(int i=0; i<2*n_aug_+1; i++)
    		x_nt_ += weights_(i)*Xsig_pred_.col(i);
  	//predicted state covariance matrix
  	P_nt_.fill(0.0);
  	for(int i=0; i<2*n_aug_+1; i++) {
    		// state difference
    		VectorXd x_diff = Xsig_pred_.col(i) - x_nt_;
    		//angle normalization
    		while (x_diff(3)> M_PI) x_diff(3)-=2.*M_PI;
    		while (x_diff(3)<-M_PI) x_diff(3)+=2.*M_PI;

    		P_nt_ += weights_(i)*x_diff*x_diff.transpose();
  	}
  }
  return;
}
